            ...
            <div class="card">
                ...
                <div class="card-body" id="{{$sub->id}}">
                 <ul>
                   ...
                   <li><img src="/images/event_photos/{{$sub->image_path}}" width="150"></li>
                   @php($dateDiff = $date_now->diffInDays($sub->event_date))
                   @if($dateDiff <= 4 && $dateDiff >= 0 )
                   <li><img class="pic" src="http://openweathermap.org/img/w/{{$weather_data[$dateDiff]['icon']}}.png" alt="{{$weather_data[$dateDiff]['icon_desc']}}" title="{{$weather_data[$dateDiff]['icon_desc']}}"> {{$weather_data[$dateDiff]['date']}} temp:{{$weather_data[$dateDiff]['temp']}} ºC  </li>
                   <li>Event date: {{$sub->event_date}}</li>

                   @endif
                   <li>Closing Subscription Date {{$sub->closing_subscription_date}}</li>
                   <li><strong><ins><br>Subscription data</ins></strong></li>   
